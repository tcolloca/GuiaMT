<?php
	...
	function connectDB($action){
		echo "Some old function trying to use SQL. Sorry about that!";
		// $DBuser = "";
		// $DBPassword = "";
		...
		// 	case "editAccount":
		// 	case "getter":
		// 		$DBuser = constant("DB_ACCOUNTS_USER");
		// 		$DBPassword = constant("DB_ACCOUNTS_PASSWORD");
		// 		break;
		// 	case "events":
		// 		$DBuser = constant("DB_EVENTS_USER");
		// 		$DBPassword = constant("DB_EVENTS_PASSWORD");
		// 		break;
		// 	case "tables":
		// 		$DBuser = constant("DB_TABLES_USER");
		// 		$DBPassword = constant("DB_TABLES_PASSWORD");
		// 		break;
		// 	case "editTable":
		// 		$DBuser = constant("DB_TABLE_EDITOR_USER");
		// 		$DBPassword = constant("DB_TABLE_EDITOR_PASSWORD");
		// 		break;
		// 	case "log":
		// 		$DBuser = constant("DB_LOG_USER");
		// 		$DBPassword = constant("DB_LOG_PASSWORD");
		// 		break;
		// }
		// $mysqli = new mysqli(constant("DB_HOST"), $DBuser, $DBPassword, constant("DB_NAME"));
	...
	function specialConnectDB($DBuser, $DBPassword){
		...
		$mysqli = new mysqli(constant("DB_HOST"), $DBuser, $DBPassword, constant("DB_NAME"));
		if ( $mysqli->connect_errno )
			echo "Fallo al conectar especial a MySQL: (" . $mysqli->connect_errno . ") " . $mysqli->connect_error;
		return $mysqli;
	}
	...
	*/
	...
	function getFirestoreTags($firestoreClient, $table, $id) {
	  return toKeyValueObject(getDocument($firestoreClient, $table."_tag", $id));
	}

	function sortTableBy($data, $col, $sortOrder) {
      $sortCol = array_column($data, $col);
      array_multisort($sortCol, $sortOrder == "desc" ? SORT_DESC : SORT_ASC, $data);
      return $data;
	}

	function doesRowMatchTags($firestoreClient, $tableName, $row, $tags_array, $condition) {
		$fullTagList = getFirestoreTags($firestoreClient, $tableName, $row[$tableName."_id"]);
		if($condition == "none" || $condition == "inone") {
			foreach($tags_array as $tag) {
				if($fullTagList["tag_".$tag] == 1) {
					return false;
				}
			}
			return true;
	...
	function filterTable($data, $filterKey, $filterValue) {
      $filteredData = array();
      foreach ($data as $row) {
    ...
    function filterTableByTags($firestoreClient, $tableName, $data, $tags_array, $condition) {
      $filteredData = array();
      foreach ($data as $row) {
        if (doesRowMatchTags($firestoreClient, $tableName, $row, $tags_array, $condition)) {
          $filteredData[] = $row;
        }
      }
      return $filteredData;
