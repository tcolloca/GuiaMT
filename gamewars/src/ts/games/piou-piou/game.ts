...
export class PiouPiou {
    ...
    coinBreakSound: Phaser.Sound;
    gameoverSound: Phaser.Sound;
    gamestartSound: Phaser.Sound;
    ...
    playAgainButton: Phaser.Group;
    backToMapButton: Phaser.Group;
    ...
    create() {
      ...
      var upKey:Phaser.Key = this.game.input.keyboard.addKey(Phaser.Keyboard.UP);
      var rightKey:Phaser.Key = this.game.input.keyboard.addKey(Phaser.Keyboard.RIGHT);
      var downKey:Phaser.Key = this.game.input.keyboard.addKey(Phaser.Keyboard.DOWN);
      var leftKey:Phaser.Key = this.game.input.keyboard.addKey(Phaser.Keyboard.LEFT);
      var rKey:Phaser.Key = this.game.input.keyboard.addKey(Phaser.Keyboard.R);
      ...
      rKey.onDown.add(() => {
        if (this.gameOver && this.game.device.desktop) this.restartGame();
      });
      ...
      var multiplier: number = this.game.device.desktop ? 1 : 2;
      ...
      var backToMapText: Phaser.Text = this.game.add.text(0, 0, "Play Again",
          { font: (24 * multiplier) + "px Arial", fill: "#ffffff" });
      ...
      var button: Phaser.Sprite = this.game.add.sprite(this.game.world.centerX, this.game.world.centerY + 250, 'button');
      button.width *= multiplier;
      button.height *= multiplier;
      button.position.x -= button.width / 2;
      button.inputEnabled = true;
    ...
    update() {
      ...
      // Blocks collision
      var collidedOnce:Boolean = false;
      for (var i = 0; i < this.blocks.length; i++) {
        for (var j = 0; j < this.blocks[i].length; j++) {
          var block = this.blocks[i][j];
          var collided:Boolean = this.game.physics.arcade.collide(this.player, block, null, null, this);
          if (collided) {
      ...
      for (var i = 0; i < this.coins.length; i++) {
        ...
        var collided:Boolean = this.game.physics.arcade.overlap(this.player, coin, null, null, this);
        if (collided) {
      ...
      for (var c = 0; c < this.coins.length; c++) {
        ...
        for (var i = 0; i < this.blocks.length; i++) {
          for (var j = 0; j < this.blocks[i].length; j++) {
            var block = this.blocks[i][j];
            var collided:Boolean = this.game.physics.arcade.collide(block, coin, null, null, this);
            if (collided) {
      ...
      if (this.game.device.desktop) this.handleDesktopInput();
      else this.handleMobileInput();
    }
    ...
    handlePlayerBlockCollision(player, block, blockData) {
      ...
      if (block.position.y + block.height < player.position.y + player.height/2) {
        ...
        if (!this.canMove || block['falling']) return;
        this.canMove = false;
        ...
        if (this.blocks[column][columnSize - 1]['falling']) columnSize--;
    ...
    spawnCoin() {
      ...
      var coin:Phaser.Sprite = this.game.add.sprite(dimBlock*i - dimCoin/2 + dimBlock/2, 0, 'coin');
    ...
    spawnBlock() {
      ...
      var block:Phaser.Sprite = this.game.add.sprite(dim*i, 0, 'block');
    ...
    checkLastRow() {
      ...
      var lastRowFilled:Boolean = true;
      for (var i = 0; i < this.blocks.length; i++) {
    ...
    toggleSpawnBlock(value:Boolean) {
      if (value) this.lastSpawnBlock = this.game.time.totalElapsedSeconds();
