...
function drawDistinctionTable() {
	...
	echo '<table id="main-table">';
		...
		$distinctions = getGameTable($firestoreClient, "twinoid", "distinction");

        if (isset($_GET["filter"]) && $_GET["filter"] != "") {
          $distinctions = filterTable($distinctions, "distinction_game", $_GET['filter']);
        }
		...
		($_GET['cat'] == "points" || $_GET['cat'] == "percentage" || $_GET['cat'] == "type" ||
		 $_GET['cat'] == "game" || $_GET['cat'] == "name")) {
            ...
            $distinctions = sortTableBy($distinctions, "distinction_" . $column, $order);
		}
		$i = 0;
		foreach($distinctions as $distinction) {
			printDistinctionData($distinction, $i++);
		}
