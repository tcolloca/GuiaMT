...
function drawMushResearchTable() {
	...
	$tags = array(
			...
			array("cat", "gato", "<b>Schrödinger</b>"),
	...
	echo '<table id="mush-table">';
		...
		$researchs = getGameTable($firestoreClient, "mush", "research");
		...
		($_GET['cat'] == "name" || $_GET['cat'] == "minefficiency" || $_GET['cat'] == "glory")) || isset($_GET['tags'])) {
			...
			if(isset($_GET['tags'])) {
				...
				$researchs = filterTableByTags($firestoreClient, "research", $researchs, $pcTags, $_GET["condition"]);
			} else {
				$column = $_GET['cat'];
				$order = $_GET['order'];

                $researchs = sortTableBy($researchs, "research_" . $column, $order);
			}
		}
		foreach($researchs as $research) {
			printMushResearchData($firestoreClient, $research, $i++, $dbTags, $fullTags);
		}
...
function printMushResearchData($firestoreClient, $research, $i, $dbTags, $fullTags) {
	...
	echo '<tr class="'.$class.'">';
		...
		echo '<td class="even" width="150px">'.getFullTags($firestoreClient, $research["research_id"], $dbTags, $fullTags).'</td>';
	echo '</tr>';
}

function getFullTags($firestoreClient, $id, $dbTags, $fullTags) {
	$resTagList = getFirestoreTags($firestoreClient, "research", $id);
	$fullTagList = array();
