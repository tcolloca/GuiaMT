...
function drawSnakeFruitTable() {
	...
	echo '<table id="snake-table">';
		...
		$fruits = getGameTable($firestoreClient, "snake", "fruit");
		...
		($_GET['cat'] == "id" || $_GET['cat'] == "name" || $_GET['cat'] == "points" || $_GET['cat'] == "vitamins"  ||
		 $_GET['cat'] == "calories" || $_GET['cat'] == "time")) || isset($_GET['tags'])) {
			...
			if(isset($_GET['tags'])) {
				...
				$fruits = filterTableByTags($firestoreClient, "fruit", $fruits, $pcTags, $_GET["condition"]);
			} else {
				$column = $_GET['cat'];
				$order = $_GET['order'];

                $fruits = sortTableBy($fruits, "fruit_" . $column, $order);
			}
		}
		foreach($fruits as $fruit) {
			printSnakeFruitData($firestoreClient, $fruit, $i++, $dbTags, $fullTags);
		}
...
function printSnakeFruitData($firestoreClient, $fruit, $i, $dbTags, $fullTags) {
	...
	echo '<td class="odd">'.$fruit["fruit_id"].'</td>';
		...
		echo '<td class="even" width="150px">'.getFullTags($firestoreClient, $fruit["fruit_id"], $dbTags, $fullTags).'</td>';
	echo '</tr>';
}

function getFullTags($firestoreClient, $id, $dbTags, $fullTags) {
	$resTagList = getFirestoreTags($firestoreClient, "fruit", $id);
	$fullTagList = array();
